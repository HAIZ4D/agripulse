...
async function test() {
  try {
    ...
    // Test the graph format { ... } without result / values
    const payload = {
        expression: {
            functionInvocationValue: {
            functionName: 'Collection.filterBounds',
            arguments: {
                collection: { functionInvocationValue: { functionName: 'ImageCollection.filterDate', arguments: { collection: { constantValue: 'COPERNICUS/S2_SR_HARMONIZED' }, start: { constantValue: dStart }, end: { constantValue: dEnd } } } },
