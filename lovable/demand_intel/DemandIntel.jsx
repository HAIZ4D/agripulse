...
import { BarChart3, Loader2, TrendingUp, Sparkles, CloudRain, Sun, Zap, Activity, DollarSign, ArrowRight } from 'lucide-react'
...
// NOTE: These imports reference your existing project components/services.
// import { useTranslation } from 'react-i18next'
// import { subscribeDemandAggregates, subscribeRecommendations, subscribePlantingPlan, acceptRecommendation, removeFromPlan, updatePlanStatus } from '../services/firebase'
...
// import DemandGap from '../components/DemandGap'
// import WeatherDemandInsight from '../components/WeatherDemandInsight'
...
export default function DemandIntel() {
  ...
  const [acceptingCrop, setAcceptingCrop] = useState(null)
  const [toast, setToast] = useState(null)

  const farmerId = 'demo'
  const areaName = 'Demo Area'

  const t = (key) => {
    const map = {
      'demandIntel.title': 'Demand Intelligence',
      'demandIntel.addedToPlan': 'Added to plan!',
      'demandIntel.removedFromPlan': 'Removed from plan',
      'demandIntel.recommendations': 'AI Recommendations',
      'demandIntel.waitingData': 'Waiting for data…',
      'common.error': 'Error',
    }
    return map[key] || key
  }
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setDemands([
        { crop: 'kangkung', totalQty: 50, buyers: 1, trend: 'stable' },
        { crop: 'chili', totalQty: 5, buyers: 1, trend: 'rising' },
      ])
      setRecommendations([])
      setPlantingPlan([])
      setLoading(false)
  ...
  async function handleAccept(rec) {
    if (!farmerId) return
    setAcceptingCrop(rec.crop)
    try {
      showToast(t('demandIntel.addedToPlan'))
    } catch (err) {
      console.error('Accept error:', err)
      showToast(t('common.error'), 'error')
    } finally {
      setAcceptingCrop(null)
    }
  }
  ...
  function handleOverride(rec) {
    const isAccepted = plantingPlan.some(
      (p) => p.crop?.toLowerCase() === rec.crop?.toLowerCase()
    )
    if (isAccepted) handleRemove(rec.crop)
  }
  ...
  return (
    <div className="min-h-screen bg-background relative">
        ...
        <div className="max-w-[1200px] mx-auto px-8 py-5 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5 flex items-center gap-1.5">
                ...
                <span className="font-semibold text-foreground/60">{areaName}</span>
              </p>
            </div>
          </div>
          <div className="hidden sm:flex items-center gap-3">
            <div className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-muted/30 border border-border/40 text-xs text-muted-foreground">
              ...
              <span>Last sync: <span className="text-foreground/70 font-medium">2m ago</span></span>
            </div>
      ...
      {/* Main Content */}
      <main className="max-w-[1200px] mx-auto px-8 py-10 space-y-8 relative z-10">
        ...
        {/* AI Forecast */}
        <Section delay={3}>
          <SectionHeading icon={Zap}>AI Forecast</SectionHeading>
          <div className="flex items-center justify-center py-6 text-center">
            <div className="flex flex-col sm:flex-row items-center gap-6">
              <div className="relative">
                <div className="absolute inset-[-10px] rounded-2xl bg-primary/8 blur-xl animate-pulse" />
              ...
              <div className="text-center sm:text-left">
                <p className="text-sm text-foreground/70 font-medium mb-1">Generate AI-powered forecast</p>
                <p className="text-xs text-muted-foreground/60 mb-4 max-w-[280px]">Predict demand trends for the next 2 weeks</p>
                <button
                  className="group relative inline-flex items-center gap-2 text-sm font-bold px-6 py-3 rounded-xl overflow-hidden transition-all duration-300 hover:scale-[1.03] active:scale-[0.98]"
                  style={{
                    background: 'linear-gradient(135deg, hsl(152, 60%, 38%), hsl(165, 50%, 42%))',
                    boxShadow: '0 4px 20px hsl(152, 60%, 42%, 0.35), inset 0 1px 0 hsl(0, 0%, 100%, 0.1)',
                  }}
                >
                  <Sparkles className="w-4 h-4 text-primary-foreground" />
                  <span className="text-primary-foreground">Generate</span>
                  <ArrowRight className="w-4 h-4 text-primary-foreground/70 transition-transform duration-300 group-hover:translate-x-0.5" />
                  <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" style={{ animation: 'shimmer 2s ease-in-out infinite' }} />
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="animate-fade-up-delay-4">
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-3">
              <span className="flex items-center justify-center w-9 h-9 rounded-xl bg-primary/15 border border-primary/20">
                <Sparkles className="w-4 h-4 text-primary" />
              </span>
              <div>
                <h2 className="text-base font-bold text-foreground">{t('demandIntel.recommendations')}</h2>
                <p className="text-xs text-muted-foreground mt-0.5">Personalized crop suggestions</p>
              </div>
            </div>
          </div>
          {recommendations.length === 0 ? (
            <div className="glow-card rounded-2xl p-12 text-center relative overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-br from-primary/[0.02] via-transparent to-secondary/[0.01] pointer-events-none" />
              <div className="relative z-10">
                <div className="relative inline-block mb-5">
                  <div className="absolute inset-[-6px] rounded-2xl bg-primary/5 blur-lg" />
                  <div className="relative w-14 h-14 rounded-2xl bg-muted/30 border border-border/30 flex items-center justify-center">
                    <Sparkles className="w-7 h-7 text-muted-foreground/20" />
                  </div>
                </div>
                <p className="text-foreground/60 font-semibold text-sm">{t('demandIntel.waitingData')}</p>
                <p className="text-xs text-muted-foreground/40 mt-1.5 max-w-[280px] mx-auto leading-relaxed">
                  AI recommendations will appear once enough market data is collected from your area.
                </p>
              </div>
            </div>
          ) : (
            <div className="space-y-4">
              {recommendations.map((rec) => {
                ...
                return crops.map((crop, i) => {
                  const isAccepted = plantingPlan.some(
                    (p) => p.crop?.toLowerCase() === crop.crop?.toLowerCase()
                  )
                  return (
                    <div key={`${rec.id}-${i}`} className="glow-card rounded-2xl p-5">
                      {/* <RecommendationCard recommendation={crop} accepted={isAccepted} loading={acceptingCrop === crop.crop} onAccept={handleAccept} onOverride={handleOverride} /> */}
                      <p>{crop.crop}</p>
                    </div>
                  )
                })
